// @ts-nocheck
// Письмо-просьба оставить отзыв на Google — вечером того же дня после визита,
// только постоянным клиентам (2+ визита). Цель: ровный ручеёк свежих отзывов в GBP.
//
...
//     DELAY=0 → окно кончается СЕГОДНЯШНИМ днём + скользящее окно догона на 5 дней
//     (кандидат, не отправленный из-за капа/сбоя, подхватится следующим прогоном);
//   - крон стоит на 20:00 Праги: письмо уходит, когда впечатление ещё свежее, но
//     все дневные чекауты уже сделаны. Кандидатом делает только status=checkedOut,
//     т.е. закрытый и оплаченный визит — «письмо клиентке, которая ещё в кресле»
//     невозможно. Визит, закрытый лишь утром следующего дня (так закрывается ~36 %
//     броней — пакетом при закрытии смены), уйдёт вечером следующего дня;
...
// через сколько дней после визита просить отзыв (0 = вечером того же дня)
const DELAY_DAYS = Math.max(0, Number(process.env.REVIEW_REQUEST_DELAY_DAYS) || 0);
